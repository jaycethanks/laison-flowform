<template>
  ...
  > -->
      ...
      <el-card class="box-card" shadow="never" :body-style="{ padding: '10px' }">
        <div style="display: flex; justify-content: space-between; align-items: center">
          <el-radio-group v-model="radio" :size="elementSize" @change="onRadioChange">
            <el-radio-button v-if="!disableDept" label="dept">{{$t('orgSelectComp.dept')}}</el-radio-button>
            <el-radio-button v-if="!disableRole" label="role">{{$t('orgSelectComp.role')}}</el-radio-button>
            <el-radio-button v-if="!disablePerson" label="person">{{$t('orgSelectComp.people')}}</el-radio-button>
          </el-radio-group>
          <el-input
            style="width: 200px"
              ...
              <el-checkbox-group ref='per' :min="minPerson" :max="maxPerson"  v-model="tree.data.person.checkedlist" style="font-size: unset;" @change="handlePersonCheckGroupChange">
                ...
                <div class="list-item" v-for="i in tree.data.person.checkSrc" :key="i.id">
                  <el-checkbox :label="i"><i :style="{color:i.sex ? '#93b5cf' : '#93b5cf',marginRight:'10px'}" class="el-icon-user-solid"></i>{{i.name}}
                    ...
                    <div v-if="i.manageFlag" class="position-label">{{$t('orgSelectComp.deptLeader')}}</div>
                </div>
              </el-checkbox-group>
            </div>
          </div>
    ...
    <span slot="footer" class="dialog-footer" v-if="selectable">
      <el-button :size="elementSize" @click="handleClose">{{$t('common.cancelText')}}</el-button>
      <el-button :size="elementSize" type="primary" @click="handleOk">{{$t('common.okText')}}</el-button>
    </span>
    <!-- </el-dialog> -->
  </a-modal>
</template>
...
export default {
  ...
  props: {
    ...
    title: {
      type: String,
      default: function(){ return this.$t('orgSelectComp.OrgStruct')},
    },
    elementSize: {
      type: String,
      default: 'mini',
    ...
    serachPlaceHolder: {
      type: String,
      default: function(){return this.$t('common.plzInput')},
    },
    tagMargin: {
      type: String,
      default: '0 5px 5px 0',
