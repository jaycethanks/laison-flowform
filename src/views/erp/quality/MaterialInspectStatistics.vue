  ...
  <div class="card-container">
    <a-card :bordered="false">
          ...
          <a-row :gutter="[24, 8]" align="middle" justify="space-between" type="flex">
            <a-col>
              <a-row :gutter="[24, 8]" align="middle" type="flex">
                <a-col>
                  <a-form-item :label="$t('InspectStatistics.materialName')">
                    <StockSelect v-model="condition.stockCode" />
                  </a-form-item>
                </a-col>

                <a-col>
                  <a-form-item :label="$t('InspectStatistics.supplierId')">
                    <SupplierSelect v-model="condition.supplierId" />
                  </a-form-item>
                </a-col>
                <a-col>
      ...
      <section class="gap--common bordered--wt section--summary">
        <a-descriptions layout="vertical" :title="$t('InspectStatistics.inspectSubtotal')" :column="2">
...
export default {
  name: 'MaterialInspectStatistics',
  ...
  data() {
    return {
      ...
      option: {
        ...
        title: { text: this.$t('InspectStatistics.materialInspectStatisticsTitle') },
        // toolbox: {
        //   feature: {
      ...
      pieoption: {
        title: { text: this.$t('InspectStatistics.materialInspectResult') },
        tooltip: {
          trigger: 'item',
        },
        legend: {
  ...
  methods: {
    moment,
    async loadData() {
      let res = await API.materialInspectStatistics(this.condition)
      this.averagepassrate = res.materialSubStatistics.avgPassPercent
      this.totalCount = res.materialSubStatistics.totalCount
