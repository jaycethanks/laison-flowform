      ...
      <div class="publish-content">
        ...
        <div class="field-item">
          <div style="float: left; width: 15%">
            ...
            <div class="inside-field-item">
              <div class="field-title">
                <div class="field-main-title required-field">{{$t('ffPublish.iconSetting')}}</div>
                <div class="field-sub-title">{{$t('ffPublish.icon')}}</div>
              </div>
          ...
          <div style="float: right; width: 75%">
            ...
            <div class="field-item">
              <div class="field-title">
                <div class="field-main-title required-field">{{$t('ffPublish.formName')}}</div>
                <div class="field-sub-title">{{$t('ffPublish.maxCharacters')}}: 20</div>
              </div>
              ...
              <div class="field-content">
                <a-input
                  allowClear
                  style="width: 100%"
                  :placeholder="$t('ffPublish.plzInputFormName')"
        ...
        <div class="field-item" style="clear: both">
          <div class="field-title">
            <div class="field-main-title required-field">{{$t('ffPublish.group')}}</div>
            <div class="field-sub-title">{{$t('ffPublish.assignAgroup')}}</div>
          </div>
          <div class="field-content" style="display: flex">
            <a-popover v-model="popoverVisible" :title="null" trigger="click">
              ...
              <!-- prettier-ignore -->
              <div style="white-space: nowrap;font-weight: 600;">{{$t('ffPublish.newGroup')}}:</div>
              <a-input @keyup.enter="handlerAdd" :maxLength="10" allowClear :placeholder="$t('ffPublish.newGroup') + ',' + $t('ffPublish.maxCharacters') + ': 10'" v-model="newGroupInputModel"></a-input>
              <!-- prettier-ignore -->
        ...
        <div class="field-item">
          <div class="field-title">
            <div class="field-main-title">{{$t('ffPublish.formDes')}}</div>
            <div class="field-sub-title">{{`${$t('ffPublish.plzInputDes')},${$t('ffPublish.maxCharacters')}: 50`}}</div>
          </div>
        ...
        <div class="field-item" v-show="showPeopleConfig">
          <div class="field-title">
            <div class="field-main-title">{{$t('ffPublish.notificateType')}}</div>
            <div class="field-sub-title">{{$t('ffPublish.howToNotice')}}</div>
          </div>

          <div class="field-content">
            <a-checkbox v-model="sumbitForm.notifyConfig.selectSMS"> {{$t('ffPublish.message')}} </a-checkbox>
            <a-checkbox v-model="sumbitForm.notifyConfig.selectEmail"> {{$t('ffPublish.email')}} </a-checkbox>
          </div>
        </div>

        <div class="field-item" v-if="showPeopleConfig">
          <div class="field-title">
            <div class="field-main-title required-field">{{$t('ffPublish.whoCanInitiate')}}</div>
            <div class="field-sub-title">{{$t('ffPublish.whoCanInitiateDes')}}</div>
          </div>
          <div class="field-content">
        ...
        <div class="field-item" v-if="showPeopleConfig">
          <div class="field-title">
            <div class="field-main-title required-field">{{$t('ffPublish.whoCanCheck')}}</div>
            <div class="field-sub-title">{{$t('ffPublish.whoCanCheckDes')}}</div>
          </div>
          <div class="field-content">
...
export default {
  ...
  computed: {
    ...
    submitBtnText: function () {
      ...
      return isPlatform ? this.$t('ffPublish.publishFlowForm') : this.$t('ffPublish.publishTemplate');
    },
  },
  watch: {
    publishEditDataInit: {
  ...
  methods: {
    ...
    handlerAdd() {
      if (!this.newGroupInputModel.trim()) {
        this.$message.error(this.$t('ffPublish.groupNameCanNotBeEmpty'));
        return;
      }
      let exist = this.groupList.some((it) => it === this.newGroupInputModel.trim());
      if (exist) {
        this.$message.error(this.$t('ffPublish.plzDontAddDuplicate'));
        return;
      }
    ...
    submit() {
      ...
      if (valid) {
        ...
        this.$message.warning(this.$t('ffPublish.requiredFieldsLack'));
      }
    },
    ...
    async submitDataJoint() {
      ...
      if (interceptingValidator()) {
        this.$message.success(this.$t('ffPublish.flowNodeCheckPassed'));
    ...
    validateNodesApproval(initDataStructure) {
      ...
      if (blankIndex != -1) {
        _this.$message.error(this.$t('ffPublish.lackApproverConfig'))
        setTimeout(() => {
          _this.jumpTo(0)
        }, 500)
        setTimeout(() => {
